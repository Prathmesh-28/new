...
router.get("/dunning/runs", async (req, res) => { try { res.json(await dunning.listRuns(tenantOf(req), { voucherId: req.query.voucherId, limit: req.query.limit })); } catch (e) { fail(res, e); } });
router.post("/dunning/runs/:id/dispatch", canPost, async (req, res) => { try { res.json(await dunning.dispatchRun(tenantOf(req), req.params.id, { channel: (req.body || {}).channel })); } catch (e) { fail(res, e); } });
...
// Business Wrapped — ledger-computed year in review.
router.get("/wrapped", async (req, res) => { try { res.json(await require("./wrapped").businessWrapped(tenantOf(req), { fy: req.query.fy })); } catch (e) { fail(res, e); } });
