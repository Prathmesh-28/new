...
export default function PayrollPage() {
  ...
  const [tab, setTab]             = useState<"employees" | "runs" | "ewa" | "slips" | "form16">("employees");
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-1 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-1 w-fit flex-wrap">
        {([["employees", `Employees (${employees.length})`, Users], ["runs", `Payroll runs (${runs.length})`, Play], ["ewa", "EWA", Banknote], ["slips", "Salary Slips", FileText], ["form16", "Form 16", FileCheck]] as const).map(([id, label, Icon]) => (
