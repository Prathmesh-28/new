...
/* Persistent global "New…" quick-create, fixed top-right just left of the notification
   ...
   (client) view. */
const ITEMS: { labelKey: string; to: string; icon: typeof Plus }[] = [
  { labelKey: "quickcreate.invoice",  to: "/invoices?compose=1", icon: FilePlus },
  { labelKey: "quickcreate.payment",  to: "/payments",           icon: IndianRupee },
  { labelKey: "quickcreate.expense",  to: "/transactions",       icon: Receipt },
  { labelKey: "quickcreate.bill",     to: "/vendors",            icon: FileText },
  { labelKey: "quickcreate.customer", to: "/sales?t=crm",        icon: UserPlus },
];
  ...
  return (
    <div ref={ref} className="fixed z-50 top-1.5 right-[6.25rem] md:top-4 md:right-[4.75rem]">
      <button onClick={() => setOpen((v) => !v)} aria-label="Create new" aria-expanded={open}
        ...
        <Plus size={16} /><span className="hidden md:inline">{t("quickcreate.new")}</span>
      </button>
