...
// from the dashboard; "Full view" returns to the normal app.
const ACTIONS = [
  { label: "New bill",       hint: "Make an invoice", to: "/invoices?compose=1", icon: FilePlus,    color: "text-blue-400",   bg: "bg-blue-950/30" },
  { label: "Got paid",       hint: "Record a payment", to: "/payments",          icon: IndianRupee, color: "text-green-400",  bg: "bg-green-950/30" },
  { label: "I spent money",  hint: "Add an expense",   to: "/transactions",      icon: Receipt,     color: "text-orange-400", bg: "bg-orange-950/30" },
  { label: "My money",       hint: "Cash & balances",  to: "/banking",           icon: Wallet,      color: "text-teal-400",   bg: "bg-teal-950/30" },
];
  ...
  return (
    <div className="min-h-[70vh] max-w-2xl mx-auto">
      ...
      <div className="rounded-2xl bg-[var(--color-surface)] border border-[var(--color-border)] p-6 text-center mb-5">
        <p className="text-sm text-[var(--color-muted)]">Money in your accounts</p>
        <p className="text-4xl font-bold mt-1 tabular-nums">{formatCurrency(balance)}</p>
      </div>

      {/* Big action buttons */}
      <div className="grid grid-cols-2 gap-4">
        {ACTIONS.map(({ label, hint, to, icon: Icon, color, bg }) => (
          <button key={label} onClick={() => navigate(to)}
            ...
            <span className="text-lg font-bold text-[var(--color-text)]">{label}</span>
      ...
      <div className="grid grid-cols-2 gap-4 mt-4">
        <button onClick={() => navigate("/alerts")}
          ...
          <span className="text-base font-semibold">Alerts{unread > 0 ? ` (${unread})` : ""}</span>
        </button>
        <button onClick={() => { document.dispatchEvent(new CustomEvent("open-headroom-assistant")); }}
          ...
          <span className="text-base font-semibold">Get help</span>
        </button>
      </div>
      ...
      <button onClick={onExit} className="mt-6 mx-auto flex items-center gap-2 text-sm text-[var(--color-muted)] hover:text-[var(--color-text)]">
        <LayoutGrid size={15} /> Switch to full view
      </button>
    </div>
  );
}
