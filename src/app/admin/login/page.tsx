...
export default function LoginPage() {
  ...
  const handleSendOtp = useCallback(async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Send via EmailJS (client-side — keeps SMTP credentials out of server bundle)
      if (EMAILJS_SERVICE_ID && EMAILJS_OTP_TEMPLATE && EMAILJS_PUBLIC_KEY) {
        await emailjs.send(
          EMAILJS_SERVICE_ID,
            ...
            {(["email", "otp"] as Step[]).map(s => (
              <div
                ...
                style={{
                  ...
                  backgroundColor: step === s ? "var(--gold)" : "var(--olive-mid)",
                }}
              />
            ))}
          </div>
